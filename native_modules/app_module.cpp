...
#endif
#include <windows.h>
#include "../../include/rayquiro/Framework.h"
#endif
...
std::string dispatch(const std::string& functionName, const std::vector<ScalarValue>& args) {
    ...
    if (functionName == "init") {
        runtime().init(args.size() > 0 ? to_string(args[0]) : "RayQuiro", args.size() > 1 ? to_int(args[1], 900) : 900, args.size() > 2 ? to_int(args[2], 600) : 600);
        return "null";
    }
    if (functionName == "run") { runtime().run(); return "null"; }
    if (functionName == "button") {
        if (args.size() < 5) return "null";
        runtime().add_button(to_string(args[0]), to_int(args[1]), to_int(args[2]), to_int(args[3]), to_int(args[4]));
        return "null";
    }
    if (functionName == "text") {
        if (args.size() < 5) return "null";
        runtime().add_text(to_string(args[0]), to_int(args[1]), to_int(args[2]), to_int(args[3]), to_int(args[4]));
        return "null";
    }
    if (functionName == "msg") {
        RayQuiroApp::show_message(args.size() > 0 ? to_string(args[0]) : "RayQuiro", args.size() > 1 ? to_string(args[1]) : "");
        return "null";
    }
#endif
    throw std::runtime_error("rayquiro.app.dll does not export app." + functionName);
}
}
